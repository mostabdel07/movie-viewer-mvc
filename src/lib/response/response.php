...
class Response
{
    ...
    // ------------------------------------------------------------------------
    function is_redirection(): bool
    {
        return  !is_null($this->redirection_path);
    }
    function has_status_code(): bool
    {
    ...
    function has_array_body(): bool
    {
        return (!is_null($this->body)) and (is_array($this->body));
    }
    function has_cookies(): bool
    {
        return (!empty($this->cookie_array));
    }
    ...
    public function send(): void
    {
        ...
        if ($this->has_cookies()) {
            $this->put_cookies_in_header();
        }
        if ($this->is_redirection()) {
            header('Location: ' . $this->redirection_path);
            return;
        }
        if ($this->has_status_code()) {
            http_response_code($this->status_code);
        }
        if ($this->has_array_body()) {
